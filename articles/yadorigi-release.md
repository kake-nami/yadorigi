...
ドラッグ＆ドロップで1件ずつ移動できるほか、**チェックボックスで複数選択して一括移動**も可能です。カードにカーソルを合わせると左上にチェックボックスが表示され、選択するとフローティングバーから移動先をワンクリックで指定できます。
...
ダッシュボード・Kanban・一覧・カテゴリ・AI検索・インポートなど、すべての画面が日本語対応しています。英語への切替も可能です。
